test_that("error handling works", {
  ...
  expect_error(PlotModificationRMSD(list(advection = 1, diffusion = 1)),
               msg, fixed = TRUE)
  expect_error(PlotModificationRMSD(list(advection = 1, diffusion = 1,
  ...
  expect_error(PlotModificationRMSD(list(advection = 1 : 5,
               ...
               msg, fixed = TRUE)
  optimum <- c(rmsd = 0, advection = 0, diffusion = 1, comprss = 2)
  expect_error(PlotModificationRMSD(list(advection = 1 : 5,
                                         diffusion = c(0.5, 1.5, 2.5),
                                         compression = c(1, 4),
                                         optimum = optimum, RMSD = 0)),
  ...
  optimum <- c(rmsd = 0, advection = 1, diffusion = 0.5, compression = 1)
  expect_error(PlotModificationRMSD(list(advection = 1 : 5,
                                         diffusion = c(0.5, 1.5, 2.5),
                                         compression = c(1, 4),
                                         optimum = optimum,
