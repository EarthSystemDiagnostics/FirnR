test_that("error handling works", {
  ...
  expect_error(CompressRecord(numeric(1), stretch = 1), msg, fixed = TRUE)
  expect_error(CompressRecord(list(depth = 1, y = 1), stretch = 1),
               msg, fixed = TRUE)
  ...
  msg <- "'stretch' needs to be of length 1."
  expect_error(
    CompressRecord(data.frame(depth = 1 : 2, y = 1 : 2), stretch = c(1, 3)),
    msg, fixed = TRUE)

  msg <- "Missing value passed for 'stretch'."
  expect_error(
    CompressRecord(data.frame(depth = 1 : 2, y = 1 : 2), stretch = NA),
    msg, fixed = TRUE)
